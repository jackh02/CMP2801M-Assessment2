...
int main()
{
	...
	while (userCommand.compare("exit") != 0)
	{
		...
		if (command.compare("addR") == 0) {
			...
			int x = int(parameters[1].c_str()); // fix me! also note that x is not previously defined :(
			int y = int(parameters[2].c_str());
			int h = int(parameters[3].c_str());
			int w = int(parameters[4].c_str());
			...
			int x = int(parameters[1].c_str());
			int y = int(parameters[2].c_str());
			int e = int(parameters[3].c_str());
